    ...
    try{ 
      ...
      if(!company){
        ...
        .populate({
          path : 'applicants',
          model: 'Candidate',
          select: '_id firstname lastname username phone about skills projects experience education achievements'
        });
        res.statusCode= 200;
        res.setHeader('Content-Type','application/json');
        res.json(applicants); 
    ...
    if(!company){    
      ...
      if(candidate){
        ...
        if(req.body.approve){
          ...
          await vacancy.save();
          await new Appointment({vacancy:req.body.vacancyId,candidate: req.body.applicantId,company: req.user._id,problem: vacancy.problem,meetinglink: "http://localhost:3000/"
          }).save();
